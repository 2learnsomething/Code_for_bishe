...
#训练结果保存
train_path = '/new_python_for_gnn/毕设code/language_model_result/train'
#测试结果保存
test_path = '/new_python_for_gnn/毕设code/language_model_result/test'
#保存路径
...
def train_model(
    ...
    if train_again:
        model.load_state_dict(
            torch.load(
                os.path.join('/new_python_for_gnn/毕设code/model_cache',
                             model_name + '.pt')))
    model.train()
        ...
        for i, (batch, price) in enumerate(zip(train_iter, train_price_iter)):
            ...
            if is_clip:
                torch.nn.utils.clip_grad_value_(
                    model.parameters(), clip_value=50)  #clip_value还需要调参.
            optimizer.step()
    ...
    print('loss saving...')
    with open(
            os.path.join(data_path, 'language_model_result/train',
...
def classification_result(confusion_matrix, cm_type, test_result, model_name):
    ...
    with open(os.path.join(result_path[1], model_name + '.json'), 'w') as f:
        f.write(json_data)
    f.close
    print('成功保存测试结果！')
...
def company_deal(train_set, test_set):
    """对训练集和测试集进一步处理

    Args:
        train_set (_type_): 测试集
        test_set (_type_): 训练集

    Returns:
        tuple: 处理之后的训练集和测试集
    """
    company_code = new_left_company(
        '/new_python_for_gnn/毕设code/news_data_from2016to2021/companies/final_use'
    )
    company_code = list(map(lambda x: x.replace('.csv', ''), company_code))
    code_name = get_company_name_code()
    company = []
    for elem in code_name:
        if elem[1] in company_code:
            company.append(elem)
    print('train_set...')
    for index, elem in tqdm(enumerate(train_set)):
        if not elem.news:
            if elem.label:
                train_set[index].news += [company[index % 1119][2], '股价', '上升']
            else:
                train_set[index].news += [company[index % 1119][2], '股价', '下降']
        else:
            vectorizer = TfidfVectorizer(max_features=500)
            corpus = ' '.join(elem.news)
            X = vectorizer.fit_transform([corpus])
            train_set[index].news = vectorizer.get_feature_names()
    print('test_set...')
    for index, elem in tqdm(enumerate(test_set)):
        if not elem.news:
            if elem.label:
                test_set[index].news += [company[index % 1119][2], '股价', '上升']
            else:
                test_set[index].news += [company[index % 1119][2], '股价', '下降']
        else:
            vectorizer = TfidfVectorizer(max_features=500)
            corpus = ' '.join(elem.news)
            X = vectorizer.fit_transform([corpus])
            test_set[index].news = vectorizer.get_feature_names()
    return train_set, test_set


def fea_select(train_set, test_set):
    """中文文本的特征选择
    ...
    """
    #vectorizer = TfidfVectorizer(max_features=500) #最多取500个特征
    vectorizer = TfidfVectorizer()
    #训练集
    for idx, elem in enumerate(train_set):
        corpus_ = ' '.join(elem.news)
        corpus = []
        corpus.append(corpus_)
        X = vectorizer.fit_transform(corpus)
        train_set[idx].news = vectorizer.get_feature_names()
    #测试集
    for idx, elem in enumerate(test_set):
        ...
        corpus.append(corpus_)
        X = vectorizer.fit_transform(corpus)
        test_set[idx].news = vectorizer.get_feature_names()
    return train_set, test_set

...
from utils_dl import train_model, test_model, get_parameter_number
from language_model.TextRNN_att import Model
...
def main():
    ...
    #模型实例化
    print('define model...')
    text_att = Model(n_vocab=n_vocab,
    ...
    print('模型结构:')
    print(text_att)
    parameters_num = get_parameter_number(text_att)
    ...
    optimizer = optim.Adam(text_att.parameters(),
                           lr=lr_,
    ...
    time5 = time.time()
    train_model(text_att, train_iter, train_price_iter, optimizer, criterion,
    ...
    print('begin tesing...')
    time7 = time.time()
    test_model(text_att, os.path.join(model_path, model_name + '.pt'),

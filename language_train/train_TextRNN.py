...
from language_model.TextRNN import Model
...
def main():
    ...
    print('模型结构:')
    print(textrnn)
    parameters_num = get_parameter_number(textrnn)
    print('模型总参数量为:')
    ...
    optimizer = optim.Adam(textrnn.parameters(),
                           lr=lr_,
    ...
    time5 = time.time()
    train_model(textrnn, train_iter, train_price_iter, optimizer, criterion,
    ...
    print('begin tesing...')
    time7 = time.time()
    test_model(textrnn, os.path.join(model_path, model_name + '.pt'),

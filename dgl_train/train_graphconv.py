...
from dgl_model.Graphconv import Model
...
def main():
    ...
    print('define model...')
    Graph_conv = Model(graph_type=graph_type,
                       n_vocab=n_vocab,
    ...
    print('模型结构:')
    print(Graph_conv)
    parameters_num = get_parameter_number(Graph_conv)
    ...
    optimizer = optim.Adam(Graph_conv.parameters(),
                           lr=lr_,
    ...
    time5 = time.time()
    train_model(Graph_conv, train_iter, train_price_iter, optimizer, criterion,
    ...
    print('begin tesing...')
    time7 = time.time()
    test_model(Graph_conv, os.path.join(model_path, model_name + '.pt'),

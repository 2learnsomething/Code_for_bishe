...
#训练结果保存
train_path = 'D:\毕设code\\technical_model_result\\train'
#测试结果保存
test_path = 'D:\毕设code\\technical_model_result\\test'
#保存路径
...
def train_pre(model, train_x, test_x, train_y, test_y):
    ...
    # 拟合的平均时间
    print("拟合的平均时间 (s):{}".format(
        round(model.cv_results_['mean_fit_time'].mean(), 3)))

    # 预测的平均时间
    print("预测的平均时间 (s):{}".format(
        round(model.cv_results_['mean_score_time'].mean(), 3)))

    best_model = model.best_estimator_
    is_pred_prob = hasattr(best_model, 'predict_prob')  #判断是否有预测分类概率的函数
    ...
    if is_pred_prob:
        pred_prob_train = best_model.predict_prob(train_x)  #获取概率
    ...
    if is_pred_prob:
        pred_prob_test = best_model.predict_prob(test_x)

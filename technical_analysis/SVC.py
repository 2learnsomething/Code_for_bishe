...
def get_x_y_data():
    ...
    train_x, test_x, train_y, test_y = train_test_split(x,
                                                        ...
                                                        shuffle=True,
                                                        random_state=1234)
    return train_x[:30000], test_x[50000:], train_y[:30000], test_y[50000:]
    ...
    for index, lable in enumerate([train_result, test_result]):
        if index == 0 and train_result[-1]:
            plot_ROC(labels=lable[0],
                     preds=lable[2],
                     savepath=os.path.join(figure_path, 'SVC_train_roc.jpg'))
        elif index == 1 and test_result[-1]:
            plot_ROC(labels=lable[0],
                     preds=lable[2],
                     savepath=os.path.join(figure_path, 'SVC_test_roc.jpg'))
        else:
